...
import { ChevronRight, Leaf, Brain, Heart, Check, Sparkles } from 'lucide-react';
...
export function OnboardingScreen({ onComplete }: OnboardingScreenProps) {
  ...
  const handleComplete = () => {
    if (name.trim() && email.trim() && password.trim() && password === confirmPassword) {
      localStorage.setItem('nourish_email', email.trim());
      if (age.trim()) {
        localStorage.setItem('nourish_age', age.trim());
      }
      ...
      <div className="flex flex-col h-full bg-[#F8FAF9]" style={{ minHeight: 'calc(100dvh - 116px)' }}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          ...
          <motion.button
            ...
            style={{
              background: name.trim() && email.trim() && password.trim() && password === confirmPassword ? 'linear-gradient(135deg, #2D6A4F, #52B788)' : '#D1D5DB',
              color: 'white',
              fontSize: '16px',
              fontWeight: 800,
              letterSpacing: '-0.2px',
              opacity: name.trim() && email.trim() && password.trim() && password === confirmPassword ? 1 : 0.6,
              boxShadow: name.trim() && email.trim() && password.trim() && password === confirmPassword ? '0 8px 24px rgba(45,106,79,0.4)' : 'none',
            }}
          >
            Start My Journey 🚀
          </motion.button>
          <p style={{ fontSize: '11px', color: '#9CA3AF', textAlign: 'center', marginTop: '12px', lineHeight: 1.5 }}>
            Your privacy matters — data stays on your device
          </p>
        </div>
      </div>
    );
