...
import {
  ...
  LogOut, Heart, Scale, Ruler, Calendar, Flame, Phone, ExternalLink, Upload, Camera,
} from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { useAuth } from './AuthContext';

const GOAL_OPTIONS = [
  { id: 'balanced', label: 'Balanced Eating', emoji: '🥗', color: '#52B788', bg: '#D8F3DC' },
  { id: 'loss', label: 'Lose Weight', emoji: '📉', color: '#F59E0B', bg: '#FEF3C7' },
  { id: 'gain', label: 'Gain Muscle', emoji: '💪', color: '#60A5FA', bg: '#DBEAFE' },
  { id: 'recovery', label: 'ED Recovery', emoji: '💙', color: '#F87171', bg: '#FEE2E2' },
];
...
export function ProfileScreen() {
  ...
  const [name, setName] = useState(localStorage.getItem('nourish_name') || 'Alex');
  const [profilePicture, setProfilePicture] = useState(localStorage.getItem('nourish_profile_picture') || '');
  const savedGoal = localStorage.getItem('nourish_goal') || 'balanced';
  const [selectedGoal, setSelectedGoal] = useState(savedGoal);
        ...
        <div className="flex-1 px-5 py-8">
          ...
          >
            ...
            <div className="space-y-4">
              {[
                {
                  icon: <Target size={20} color="#2D6A4F" />,
                  title: 'Track Your Progress',
                  desc: 'Monitor your nutrition goals and achievements',
                  bg: '#D8F3DC',
                },
                {
                  icon: <Heart size={20} color="#F87171" />,
                  title: 'Personalized Support',
                  desc: 'Get tailored recommendations for your wellness journey',
                  bg: '#FEE2E2',
                },
                {
                ...
                <div key={title} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 pt-1">
                    <p style={{ fontSize: '14px', fontWeight: 700, color: '#1B4332', marginBottom: '2px' }}>
                      {title}
                    </p>
                    <p style={{ fontSize: '12px', fontWeight: 500, color: '#6B7280', lineHeight: 1.4 }}>
                      {desc}
                    </p>
                  </div>
                </div>
    ...
    <div className="flex flex-col bg-[#F8FAF9]">
      ...
      <div className="px-4 py-4">
        ...
        <div className="rounded-3xl bg-white p-4 mb-3" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          {[
            { icon: <Flame size={16} color="#F59E0B" />, label: 'Calorie Goal', value: '1,800 kcal', accent: '#F59E0B' },
            { icon: <span style={{ fontSize: '16px' }}>💪</span>, label: 'Protein Goal', value: '120g', accent: '#60A5FA' },
            { icon: <span style={{ fontSize: '16px' }}>🌾</span>, label: 'Carbs Goal', value: '225g', accent: '#F59E0B' },
            { icon: <span style={{ fontSize: '16px' }}>💧</span>, label: 'Water Goal', value: '8 glasses', accent: '#60A5FA' },
        ...
        <div className="rounded-3xl bg-white p-4 mb-3" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <ToggleRow
            ...
            subtitle="Daily mood & energy check"
            defaultOn={true}
            accent="#F87171"
          />
          <ToggleRow
            icon={<Target size={16} color="#52B788" />}
            label="Goal Alerts"
            subtitle="Notify when close to or over goal"
            defaultOn={false}
            accent="#52B788"
          />
          <ToggleRow
        ...
        <div className="rounded-3xl bg-white p-4 mb-3" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <ToggleRow
            ...
            subtitle="Remove personal identifiers"
            defaultOn={false}
            accent="#6B7280"
          />
          {[
        ...
        >
          ...
          <div className="flex flex-col gap-2">
            {ED_RESOURCES.map(({ name, detail, color, bg, icon }) => (
              <div
                key={name}
              ...
              >
                ...
                <div className="flex-1">
                  <p style={{ fontSize: '12px', fontWeight: 800, color }}>{name}</p>
